 ...
 *
 * (Hacked a tiny bit to get rid of some GCC warnings.)
 */
...
uint8_t minwidth(int32_t value)
{
        int bits = 32;
        uint32_t mask = 1 << 31;
...
uint8_t minw_notmarker_8(int32_t value)
{
        uint8_t width = minwidth(value);
        if (width < 7) {
...
int itsex_decompress8 (FILE *module, void *dst, int len, int it215)
{
	...
	while (blkpos < blklen) {
		...
		freeblock ();
		fprintf(stderr, "illegal bit width!\n");
		return 0;
	    }
...
int main(int argc, char **argv)
{
        ...
        if (!(sample.flags & FLG_COMPR)) {
                fprintf(stderr, "input sample is not compressed\n");
                return 1;
        }

        it215 = sample.cvt & CVT_DELTA;
        sample.flags &= ~FLG_COMPR;
        sample.cvt &= ~CVT_DELTA;

        smpwidth = (sample.flags & FLG_16BIT) ? 2 : 1;
        buf = calloc(sample.length, smpwidth);
        if (!buf) {
                fprintf(stderr, "out of memory!\n");
                err = 1;
        } else if (fwrite(&sample, sizeof(sample), 1, stdout) != 1) {
                fprintf(stderr, "write error\n");
                err = 1;
        } else if (!(((sample.flags & FLG_16BIT)
                      ? itsex_decompress16
                      : itsex_decompress8)
                     (stdin, buf, sample.length, it215))) {
                fprintf(stderr, "decompression failure\n");
                err = 1;
        } else if (fwrite(buf, sample.length, smpwidth, stdout) != smpwidth) {
                fprintf(stderr, "write error\n");
                err = 1;
        }
        free(buf);
